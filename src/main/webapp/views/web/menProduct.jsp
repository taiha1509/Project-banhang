					...
					<c:forEach var="j" begin="0" end="2">
						<div class="col-lg-4 mb-4">
							<div class="card h-100">
								<img class="card-img"
									src="Image/Product/${product.get(i*3+j).get(0).image}">
								<div class="card-body">
									<h4>${product.get(i*3 +j).get(1).productName}</h4>
								</div>
								<div class="card-footer">

									<p class="text-color-secondary ">${product.get(i*3 +j).get(1).price} ${product.get(i*3 +j).get(1).currency}</p>
									<a class="add_to_cart_button text-center" data-quantity="1"
										data-product_sku="" data-product_id="70" rel="nofollow"
										href="/single_product/${product.get(i*3+j).get(1).productId}">Detail</a>
								</div>
							</div>
						</div>
					</c:forEach>
