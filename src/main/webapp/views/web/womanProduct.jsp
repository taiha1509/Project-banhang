...
.sticky {
	...
	<div class="row">
		...
		<div class="col-lg-9">
			<br>
			<h2 class="related-products-title">Product-Woman</h2>


			<!-- Marketing Icons Section -->
			<c:forEach var="i" begin="0" end="${(sl+2)/3 - 1}">
				<div class="row">
					<c:forEach var="j" begin="0" end="2">
						<c:if test="${sl >= (i*3+j+1) }">
							<div class="col-lg-4 mb-4">
								<div class="card h-100">
									<img class="card-img"
										src="Image/Product/${proWoman.get(i*3+j).get(0).image}">
									<div class="card-body">
										<h4>${proWoman.get(i*3 +j).get(1).productName}</h4>
									</div>
									<div class="card-footer">

										<p class="text-color-secondary ">${proWoman.get(i*3 +j).get(1).price}
											${proWoman.get(i*3 +j).get(1).currency}</p>
										<a class="add_to_cart_button text-center" data-quantity="1"
											data-product_sku="" data-product_id="70" rel="nofollow"
											href="/single_product/${proWoman.get(i*3+j).get(1).productId }">Detail</a>
									</div>
								</div>
							</div>
						</c:if>
