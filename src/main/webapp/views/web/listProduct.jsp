...
.sticky {
	...
	<div class="row">
		...
		<div class="col-lg-9">
			<br>
			<h2 class="related-products-title">Total Product: ${len }</h2>
			<c:if test="${empty proSearch }">
				<h1 class="text-warning">there is nothing like that</h1>
			</c:if>

			<!-- Marketing Icons Section -->
			<c:if test="${not empty proSearch}">
				<c:forEach var="i" begin="0" end="${numrow - 1}">
					<div class="row">
							...
							<c:if test="${len > (j+i*3)}">
								<div class="col-lg-4 mb-4">
									<div class="card h-100">
										<img class="card-img"
											src="Image/Product/${proSearch.get(i*3+j).get(0).image}">
										<div class="card-body">
											<h4>${proSearch.get(i*3 +j).get(1).productName}</h4>
										</div>
										<div class="card-footer">

											<p class="text-color-secondary ">${proSearch.get(i*3 +j).get(1).price} ${proSearch.get(i*3 +j).get(1).currency}</p>
											<a class="add_to_cart_button text-center" data-quantity="1"
												data-product_sku="" data-product_id="70" rel="nofollow"
												href="/single_product/${proSearch.get(i*3+j).get(1).productId} ">Detail</a>
										</div>
									</div>
								</div>
							</c:if>
